...
handlers._checks.post = function(data, callback) {
    ...
    id (!(protocol && url && method && successCodes && timeoutSeconds))
    if (!(phone && password)) {
        callback(400, {"error":"Missing required field(s)"});
        return;
    }
    ...
    _data.read("tokens", token, function (err, resultData) {
        if (!err && resultData) {
            ...
            _data.read('users', userPhone, function(err, userData) {
                var userChecks = [];
                if (!err && userData) {
                    if (typeof(userData.userChecks) == 'object' && userData.userChecks instanceof Array) {
                        userChecks = userData.userData;
                    }
                } else {
                    callback(403);
                }
